    ...
    <div className="relative mx-auto max-w-[1400px] px-3 pt-3 sm:px-4 md:px-8 md:pt-5">
      <div className="relative overflow-hidden rounded-[28px] border border-[rgba(232,226,217,.9)] bg-[rgba(255,252,247,.92)] shadow-[0_24px_70px_rgba(42,33,24,.08)]">
        ...
        <div className="relative min-h-[560px] lg:grid lg:min-h-[650px] lg:grid-cols-[.78fr_1.22fr]">
          <div className="relative z-10 px-6 pb-7 pt-14 md:px-10 lg:flex lg:flex-col lg:justify-center lg:px-14 lg:py-16 hidden lg:flex">
          ...
          <div className="absolute inset-0 overflow-hidden lg:relative lg:w-auto lg:min-h-[650px]">
            <img src="/hero-mobile.webp" alt="" className="h-full w-full object-cover lg:hidden" style={{ objectPosition: '78% center' }} />
            <img src={heroImage} alt="QUORIN creative supplies" className="absolute inset-0 hidden h-full w-full object-cover lg:block" style={{ objectPosition: 'center 48%' }}/>
            <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(250,247,240,0.98)_0%,rgba(250,247,240,0.92)_28%,rgba(250,247,240,0.35)_54%,rgba(250,247,240,0.08)_100%)] lg:hidden" />
            <div className="relative z-10 flex h-full flex-col px-6 pt-10 pb-8 lg:hidden">
              <div className="w-[58%] max-w-[220px]">
                <motion.p className="mb-4 flex items-center gap-3 text-[8px] font-medium uppercase tracking-[.48em] text-[#3D3731] before:h-px before:w-8 before:bg-[#C9A96E]/65 after:h-px after:w-8 after:bg-[#C9A96E]/65" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>Made for Makers</motion.p>
                <motion.h1 className="quorin-brand text-[clamp(3.65rem,16vw,5.3rem)] font-normal leading-[.84] tracking-[.02em] text-[#C9A35D]" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>QUORIN</motion.h1>
                <p className="mt-5 text-[14px] leading-[1.58] text-[#3E3730]">Premium crafting supplies for resin art,<br/> candle making, and soap making.<br/><span className="font-medium text-[#C99A4D]">Everything you need to bring your creative vision to life.</span></p>
                <div className="mt-7 flex flex-col gap-3"><button onClick={() => navigate('/search')} className="inline-flex h-12 items-center justify-center gap-3 rounded-full bg-[#D0A85A] px-6 text-[10px] font-semibold uppercase tracking-[.2em] text-white">Explore Collection <ArrowRight size={16}/></button><button onClick={() => navigate('/search')} className="inline-flex h-12 items-center justify-center rounded-full border border-[#D0A85A] px-6 text-[10px] font-semibold uppercase tracking-[.2em] text-[#B98C43]">Shop Best Sellers</button></div>
              </div>
            </div>
          </div>
        </div>
